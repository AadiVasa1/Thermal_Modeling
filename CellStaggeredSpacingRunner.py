...
c = cs.CellStaggeredSpacing(14, 18.6e-3)
...
for j in SL:  # Outer loop for rows
    ...
    for i in ST:  # Inner loop for columns
        ...
        hh, p = c.known_v_and_spacing(1.93, i, j)
        row.append(hh/p)
...
t, l, _ = c.known_v_optimize_spacing(1.93, [.02, .025], [.0174, .025], 74, .0001, .0001)
print(c.known_v_and_spacing(1.93, t, l))
print(c._v_max(.0186, .02, .0174, 1.93))
